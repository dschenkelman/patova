...
function setupRateLimitEventExt(server, options) {
  ...
  server.ext(event, (request, reply) => {
    //According to HAPI documentation,
    //this should be emitted but it is not.
    //https://github.com/hapijs/hapi/issues/3528
    // request.once('disconnect', () => {
    //   store.getAndRemove(request.id);
    // });
    if (request.raw && request.raw.req) {
...
function setupCleanup(server, options) {
  // Handles disconnects and client errors
...
exports.register = function (server, options, next) {
  ...
  Joi.validate(options, schema, { abortEarly: false }, (err, processedOptions) => {
    ...
    setupCleanup(server, processedOptions);
    next();
  });
