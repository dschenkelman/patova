...
function buildRateLimitEvent({ event, extractKey, limitd, onError, type }) {
...
async function register(server, options) {
  const { error, value }= await Joi.validate(options, schema, { abortEarly: false });
  ...
  server.ext(options.event, buildRateLimitEvent(value));
}
